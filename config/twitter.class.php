...
/// reply
	...
	public function reply($message,$tweetid)
	{
		if (iconv_strlen($message, 'UTF-8') > 140) {
			$message = preg_replace_callback('#https?://\S+[^:);,.!?\s]#', array($this, 'shortenUrl'), $message);
		}

		@$res = $this->request('statuses/update', array('status' => $message,'in_reply_to_status_id' => $tweetid));
		if(@$res->id) {return @$res->id;}else { return 0;}
	}

	public function follow($screen_name,$user_id,$follow)
	{
		@$res = $this->request('friendships/create', array('screen_name' => $screen_name,'user_id' => $user_id, 'follow' => $follow));
		if(@$res->id) {return @$res->id;}else { return 0;}
	}
	/**
